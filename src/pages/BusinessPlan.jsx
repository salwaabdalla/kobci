...
const QUESTIONS = [
  ...
  { field: 'targetCustomer', label: 'Macaamiishaada Bartilmaameedka', placeholder: 'Cid kale ee aad u iibisid?' },
...
Kala duwaanida Tartanka: ${form.competitiveAdvantage}
Bartilmaameedka Dakhliga Bishii: ${form.monthlyGoal} shilin
Caqabadda ugu Weyn: ${form.biggestChallenge}

Qorshaha waa inuu ka kooban yahay: 1) Soo-koobid, 2) Cilmi baarista suuqa, 3) Alaabta/Adeegga, 4) Qorshaha Suuqgeynta, 5) Qorshaha Maaliyadda, 6) Hadafyada 6-bilood. Soomaali buuxda ah si qumman u qor.`
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `Qorshaha-${form.businessName || 'Ganacsiga'}.txt`
    a.click()
      ...
      {!plan ? (
        <div className="space-y-4">
          <p className="text-muted text-sm">
            Buuxi su'aalaha hoose, Kobcin wuxuu kuu qori doonaa qorshe ganacsi oo buuxa.
          </p>

          <div className="space-y-3">
            {QUESTIONS.map(q => (
              <div key={q.field}>
                <label className="text-sm font-medium text-brown mb-1 block">{q.label}</label>
                <input
                  value={form[q.field]}
                  onChange={e => setForm({ ...form, [q.field]: e.target.value })}
          ...
          <button
            ...
            disabled={loading || !form.businessName || !form.whatYouSell}
        ...
        <div className="space-y-4">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-heading font-bold text-brown text-lg">
                Qorshaha Ganacsiga — {form.businessName}
              </h2>
              <button
                onClick={() => {
                  setPlan('')
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => {
                if (navigator.share) {
                  navigator.share({ title: `Qorshaha ${form.businessName}`, text: plan })
                }
              }}
