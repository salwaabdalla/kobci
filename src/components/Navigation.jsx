...
export default function Navigation() {
  const { userProfile, streak } = useApp()
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${isActive ? activeClass : inactiveClass}`
              }
            >
